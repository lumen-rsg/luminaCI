  ...
  return <>
    ...
    {state.loading ? <Loading label="Loading security posture" /> : state.error || !state.data ? <ErrorState error={state.error} retry={state.reload} /> : <div className="page-stack">
      ...
      <section className="panel table-panel">
        ...
        {scans.length ? <div className="table-wrap"><table><caption className="sr-only">Recent vulnerability scans</caption><thead><tr><th>Artifact</th><th>Scanner</th><th>Status</th><th>Findings</th><th>Completed</th></tr></thead><tbody>{scans.map(scan => <tr key={scan.id}><th><code>{shortId(scan.artifactId)}</code></th><td>{scan.scannerType}</td><td><Status value={scan.status} /></td><td><span className={scan.criticalCount ? "risk risk--critical" : "risk"}>{scan.criticalCount} critical</span><span className={scan.highCount ? "risk risk--high" : "risk"}>{scan.highCount} high</span></td><td>{dateTime(scan.completedAt || scan.createdAt)}</td></tr>)}</tbody></table></div> : <Empty title="No scans recorded" detail="Vulnerability results appear after an artifact reaches the scan stage." />}
      </section>
      <section className="panel table-panel">
