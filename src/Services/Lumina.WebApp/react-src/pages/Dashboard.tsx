  ...
  return (
    <div className="page-stack">
        ...
        <article className="panel panel--wide">
          ...
          {data.builds.length ? <div className="activity-list">{data.builds.slice(0, 6).map((build: any) =>
            <Link to={`/builds/${build.id}`} className="activity-row" key={build.id}>
              ...
              <Status value={build.status} domain="build" /><time>{timeAgo(build.createdAt)}</time><ArrowRight className="row-arrow" />
      ...
      <section className="panel">
        ...
        <div className="pipeline-strip">{data.pipelines.slice(0, 4).map((pipeline: any) => <article key={pipeline.id}><div><GitBranch /><Status value={pipeline.status} domain="pipeline" /></div><h3>{pipeline.name}</h3><p>{pipeline.description || "No description provided."}</p><small>{pipeline.stepCount} steps · {pipeline.createdBy}</small></article>)}</div>
      </section>
    </div>
  );
}
