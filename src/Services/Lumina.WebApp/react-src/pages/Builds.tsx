      ...
      {state.loading ? <Loading label="Loading build history" /> : state.error ? <ErrorState error={state.error} retry={state.reload} /> : builds.length ? (
        <div className="panel table-panel">
          <div className="table-wrap"><table><caption className="sr-only">Build history</caption><thead><tr><th>Build</th><th>Status</th><th>Pipeline</th><th>Triggered by</th><th>Created</th><th aria-label="Actions" /></tr></thead>
            <tbody>{builds.map(build => <tr key={build.id}><th scope="row"><Link to={`/builds/${build.id}`}><span className="table-title">{build.specName}</span><code>{shortId(build.id)}</code></Link></th><td><Status value={build.status} /></td><td><code>{shortId(build.pipelineId)}</code></td><td>{build.triggeredBy}</td><td title={build.createdAt}>{timeAgo(build.createdAt)}</td><td><Link className="icon-button" to={`/builds/${build.id}`} aria-label={`Inspect ${build.specName}`}><ChevronRight /></Link></td></tr>)}</tbody>
          </table></div>
