...
export function Sources() {
  ...
  return <>
    ...
    {state.loading ? <Loading label="Loading source catalog" /> : state.error ? <ErrorState error={state.error} retry={state.reload} /> : packages.length ? <div className="source-list">{packages.map(source => <article className="source-card" key={source.packageId}>
      ...
      <div className="source-card__main"><div><h2>{source.packageName}</h2><span className="revision">r{source.revision}</span><Status value={source.status} domain="source" /></div><p title={source.sourceUrl}>{source.sourceUrl}</p><small>{source.sourceBranch || "SHA-256 pinned"} · {source.lastFetchedAt ? `Fetched ${dateTime(source.lastFetchedAt)}` : "Never fetched"}</small></div>
...
function SourceModal({ source, onClose, onSaved }: { source?: SourcePackage; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title={source ? `Edit ${source.packageName}` : "Add package source"} onClose={onClose}><form className="form-grid" onSubmit={submit}>
    ...
    <label>Source type<select value={draft.sourceType} onChange={event => setDraft({ ...draft, sourceType: event.target.value })}><option value="0">Git</option><option value="1">Tar</option><option value="2">HTTP</option></select></label>
